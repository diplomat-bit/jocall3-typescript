...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['JOCALL3_LOG'] or 'warn' if it isn't set.
   */
...
export class Jocall3 {
   ...
   */
  constructor({
    baseURL = readEnv('JOCALL3_BASE_URL'),
    apiKey = readEnv('JOCALL3_API_KEY') ?? null,
    geminiAPIKey = readEnv('GEMINI_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Jocall3.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('JOCALL3_LOG'), "process.env['JOCALL3_LOG']", this) ??
      defaultLogLevel;
